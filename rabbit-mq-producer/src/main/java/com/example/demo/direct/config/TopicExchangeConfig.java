...
public class TopicExchangeConfig {
	...
	public static final String EXCHANGE_NAME="tp.awards_exchange";
	...
	public Binding tpbinding1() {
		...
		return BindingBuilder.bind(queue1()).to(topicExchange()).with("*.cse.*");
	}
	
	@Bean
	public Binding tpbinding2() {
		
		return BindingBuilder.bind(queue2()).to(topicExchange()).with("*.cse.*");
	}
	
	@Bean
	public Binding tpbinding3() {
		
		return BindingBuilder.bind(queue3()).to(topicExchange()).with("*.ece.*");
	}
